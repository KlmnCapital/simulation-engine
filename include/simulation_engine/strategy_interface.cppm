...
template <std::size_t depth, std::uint16_t numberOfSymbols, typename Distribution> class Engine;
...
*/
template <std::size_t depth, std::uint64_t numberOfSymbols, typename Distribution>
class IStrategy {
public:
    ...
    void setEngine(Engine<depth, numberOfSymbols, Distribution>* engine) {
        engine_ = engine;
    }
    ...
    OrderId placeOrder(std::uint16_t symbol,
        ...
        pendingOrders.push_back({
            .order = newOrder,
            .sendTime = engine_->currentTime(), 
            .earliestExecution = engine_->currentTime() // Adjust for latency if needed
        });
    ...
    const Portfolio<numberOfSymbols, Distribution>& portfolio() const {
